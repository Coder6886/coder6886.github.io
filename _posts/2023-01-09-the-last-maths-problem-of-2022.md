...
f(x)=\frac{1}{q}(p_nx^n+p_{n-1}x^{n-1}+...+p_1x+p_0)\quad(p_0,p_1,...,p_n)=1,p_k,q\in\mathbb{Z},k\in\mathbb{Z}
...
一方面，定义$w_n(x)=C_x^n=\frac{x(x-1)(x-2)...(x-n+1)}{n!}$，它在所有整数$x$上为整数（它在0,1,2,...,n-1上为0，在n上为1，所以根据定理1，可以知道$C_x^n$在所有整数$x$上都为整数），且它的$x^n$项为$\frac{1}{n!}$，所以$q_k=n!$可以取到。

另一方面，设$g^k_f(x)=g^{k-1}_f(x+1)-g^{k-1}_f(x)(k>0)$，且$g_f^0(x)=f(x)$，可以知道$g_f^n(x)$为常函数。
...
对于两个一元$n$次函数$f(x)$和$h(x)$，如果有$\forall 0\leq k\leq n,g_f^k(0)=g_{h}^k(0)$则$f(x)=h(x)$

## 引理3证明

我们知道所有$g^k_f(0)\,(0\leq k\leq n)$，那我们就知道了$g^n_f(x)=g^n_f(0)$（它是常函数），那我们就可以根据 
$$
g^{k-1}_f(x)=g^k_f(x-1)+g^{k-1}_f(x-1)
$$
递推出所有$g^k_f(x)$，也就能唯一确定$f(x)$。然而 $\forall 0\leq k\leq n,g_f^k(0)=g_{h}^k(0)$，所以$g_f^k(0)$同样可以确定函数$h(x)$。即$f(x)$必然等于$h(x)$。

## 最后证明

我们知道$g^k_{w_c}(0)=0(0\leq k<c),g^c_{w_c}(0)=1$ （这是一个简单计算，用到了$w_c(0)=0,w_c(1)=0,..w_c(c-1)=0,w_c(c)=1$ ）

设$h(x)=g_f^0(0)C_x^0+g_f^1(0)C^1_x+...+g^n_f(0)C^n_x$，则由引理1,引理2可得

$$

g_{h}^k(0)=g^0_f(0)\times 0+g_f^1(0)\times0+..+g_f^{k-1}(0)\times 0+g_f^k(0)\times1+g_f^{k+1}(0)\times0+...+g_f^n(0)\times0
...
然而$g^k_f(0)$都是整数（显而易见），所以$q$最大为$w_c$的分母的最小公倍数，也就是$n!$。命题得证■
